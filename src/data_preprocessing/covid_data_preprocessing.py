...
class DataPreprocessing:
    ...
    def __init__(self, data_paths):
        ...
        # self.google_mobility_report = pd.read_csv(
        #     "./data/Updated Data/mobility/Google/Global_Mobility_Report.csv"
        # )
    ...
    def create_state_mapping(self):
        ...
        for state_name in state_names:
            mapping_dict[state_name] = self.us_testing.loc[
                self.us_testing["state"] == state_name
            ]["state_name"].iloc[0]
        return mapping_dict

    @staticmethod
    def data_imputation(
        state, state_dataframe, imputation_columns, data_type, booster_data
    ):
                ...
                for i in range(len(state_dataframe)):
                    if str(state_dataframe[column_name][i]) == "nan":
                        ...
                        for j in range(counter):
                            state_dataframe[column_name][i + j] = state_dataframe[
                                column_name
                            ][i + j + counter]
                    else:
                        break
            ...
            for column_name in imputation_columns:
                if str(state_dataframe[column_name][i]) == "nan":
                    ...
                    while str(state_dataframe[column_name][i + counter]) == "nan":
                        counter += 1
                    diff = (
                        state_dataframe[column_name][i + counter]
                        - state_dataframe[column_name][i - 1]
                    )
                    for j in range(counter):
                        try:
                            if data_type == "int":
                                state_dataframe[column_name][i + j] = int(
                                    state_dataframe[column_name][i + j - 1]
                                    + diff / (counter + 1)
                                )
    ...
    def create_state_vaccination_data(self):
        ...
        for state in states:
            ...
            state_vaccination = self.us_vaccinations.loc[
                self.us_vaccinations["location"] == state
            ]
            ...
            self.data_imputation(
                state=state,
                state_dataframe=state_vaccination,
            ...
            self.data_imputation(
                state=state,
                state_dataframe=state_vaccination,
                imputation_columns=booster_columns,
                data_type="int",
                booster_data=True,
            )

            state_vaccination.to_csv(
                f"./data/state_vaccinations/{state}_vaccination.csv", index=False
            )
    ...
    def create_state_hospitalization_data(self):
        ...
        for state in states:
            state_hospitalization = self.us_hospitalizations.loc[
                self.us_hospitalizations["state"] == state
            ]
            ...
            state_hospitalization.to_csv(
                f"./data/state_hospitalizations/{state}_hospitalization.csv",
                index=False,
            )
    ...
    def create_state_testing_data(self):
        ...
        for state in states:
            ...
            state_dataframe.to_csv(
                f"./data/state_testing/{state}_testing.csv", index=False
            )
    ...
    def create_state_final_dataset():
        ...
        population_dynamics = population_dynamics.merge(
            vaccination_data, how="inner", on="date"
        )
        population_dynamics["date"] = pd.to_datetime(population_dynamics["date"])
        population_dynamics = population_dynamics.merge(
            hospitalization_data, how="inner", on="date"
        )
        population_dynamics = population_dynamics.merge(
            testing_data, how="inner", on="date"
        )
...
data_paths = {
    "testing": f"{DATA_DIR}/testing/usa_testing.csv",
    "hospitalization": f"{DATA_DIR}/hospitalization/usa_hospitalization.csv",
    "vaccination": f"{DATA_DIR}/hospitalization/usa_hospitalization.csv",
    "google_mobility_report": f"{DATA_DIR}/hospitalization/usa_hospitalization.csv",
}

data_preprocessing = DataPreprocessing(data_paths=data_paths)
...
epidemiological_model_data = epidemiological_model_data.merge(
    updated_ny, how="inner", on="date"
)
epidemiological_model_data.to_csv(
    "./updated_epidemiological_model_data.csv", index=False
)
