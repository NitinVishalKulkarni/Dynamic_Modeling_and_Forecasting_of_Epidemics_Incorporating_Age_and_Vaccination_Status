...
class DataPreprocessing:
                ...
                for i in range(len(data) - 0):
                    if pd.isnull(data[column_name][i]):
                        ...
                        for j in range(counter):
                            try:
                                data[column_name][i + j] = data[column_name][
                                    i + counter
                                ]
                            except KeyError:
                                print(i, j, counter)
    ...
    def preprocess_data_by_vaccination_status(self):
        ...
        def helper_preprocess_data_by_vaccination(
            data, earliest_date, latest_date, output_file_name
        ):
            ...
            data["week_end_date"] = data["MMWR week"].apply(
        ...
        cases_by_vaccination = self.cases_deaths_by_age_vaccination.loc[
            (self.cases_deaths_by_age_vaccination["outcome"] == "case")
            & (self.cases_deaths_by_age_vaccination["Age group"] == "all_ages_adj")
            & (self.cases_deaths_by_age_vaccination["Vaccine product"] == "all_types")
        ]

        helper_preprocess_data_by_vaccination(
            data=cases_by_vaccination,
            earliest_date="04/04/2021",
            latest_date="09/24/2022",
            output_file_name="cases_by_vaccination.csv",
        )

        # Deaths by vaccination
        deaths_by_vaccination = self.cases_deaths_by_age_vaccination.loc[
        ...
        helper_preprocess_data_by_vaccination(
            data=deaths_by_vaccination,
            earliest_date="04/04/2021",
            latest_date="09/03/2022",
            output_file_name="deaths_by_vaccination.csv",
        )

        def helper_preprocess_data_by_first_booster(
            data, earliest_date, latest_date, output_file_name
        ):
            ...
            for i in range(len(data)):
                ...
                unvaccinated_multiplier = 1 / (
                    1 + (1 / age_adjusted_irr) + (1 / age_adjusted_booster_irr)
                )
                data["UV_multiplier"].iloc[i] = unvaccinated_multiplier
                primary_series_vaccinated_multiplier = 1 / (
                    age_adjusted_irr
                    + 1
                    + (1 / (age_adjusted_booster_irr / age_adjusted_irr))
                )
                data["PSV_multiplier"].iloc[i] = primary_series_vaccinated_multiplier
                first_booster_vaccinated_multiplier = 1 / (
                    age_adjusted_booster_irr
                    + (age_adjusted_booster_irr / age_adjusted_irr)
                    + 1
                )
                data["BV1_multiplier"].iloc[i] = first_booster_vaccinated_multiplier

            data["week_end_date"] = data["mmwr_week"].apply(
                lambda x: epiweeks.Week.fromstring(str(x)).enddate()
            )
        ...
        cases_by_first_booster = self.cases_deaths_by_age_first_booster.loc[
            (self.cases_deaths_by_age_first_booster["outcome"] == "case")
            & (self.cases_deaths_by_age_first_booster["age_group"] == "all_ages")
            & (self.cases_deaths_by_age_first_booster["vaccine_product"] == "all_types")
        ]
        helper_preprocess_data_by_first_booster(
            data=cases_by_first_booster,
            earliest_date="09/19/2021",
            latest_date="09/24/2022",
            output_file_name="cases_by_first_booster.csv",
        )

        deaths_by_first_booster = self.cases_deaths_by_age_first_booster.loc[
            (self.cases_deaths_by_age_first_booster["outcome"] == "death")
            & (self.cases_deaths_by_age_first_booster["age_group"] == "all_ages")
            & (self.cases_deaths_by_age_first_booster["vaccine_product"] == "all_types")
        ]
        helper_preprocess_data_by_first_booster(
            data=deaths_by_first_booster,
            earliest_date="09/19/2021",
            latest_date="09/03/2022",
            output_file_name="deaths_by_first_booster.csv",
        )

        def helper_preprocess_data_by_second_booster(
            data, earliest_date, latest_date, output_file_name
        ):
            """This is a helper function to preprocess the data on first booster vaccination,
            primary series vaccination, and unvaccinated individuals."""
            ...
            for i in range(len(data)):
                ...
                age_adjusted_second_booster_irr = data["age_adj_two_booster_irr"].iloc[
                    i
                ]

                unvaccinated_multiplier = 1 / (
                    1
                    + (1 / age_adjusted_vax_irr)
                    + (1 / age_adjusted_first_booster_irr)
                    + (1 / age_adjusted_second_booster_irr)
                )
                data["UV_multiplier"].iloc[i] = unvaccinated_multiplier

                primary_series_vaccinated_multiplier = 1 / (
                    age_adjusted_vax_irr
                    + 1
                    + (1 / (age_adjusted_first_booster_irr / age_adjusted_vax_irr))
                    + (1 / (age_adjusted_second_booster_irr / age_adjusted_vax_irr))
                )
                data["PSV_multiplier"].iloc[i] = primary_series_vaccinated_multiplier

                first_booster_vaccinated_multiplier = 1 / (
                    age_adjusted_first_booster_irr
                    + (age_adjusted_first_booster_irr / age_adjusted_vax_irr)
                    + 1
                ...
                second_booster_vaccinated_multiplier = 1 / (
                    age_adjusted_second_booster_irr
                    + (age_adjusted_second_booster_irr / age_adjusted_vax_irr)
                    + (age_adjusted_second_booster_irr / age_adjusted_first_booster_irr)
                    + 1
                )
        ...
        cases_by_second_booster = self.cases_deaths_by_age_second_booster.loc[
            (self.cases_deaths_by_age_second_booster["outcome"] == "case")
            & (self.cases_deaths_by_age_second_booster["age_group"] == "all_ages")
            & (
                self.cases_deaths_by_age_second_booster["vaccine_product"]
                == "all_types"
            )
        ]
        helper_preprocess_data_by_second_booster(
            data=cases_by_second_booster,
            earliest_date="03/27/2022",
            latest_date="09/24/2022",
            output_file_name="cases_by_second_booster.csv",
        )

        deaths_by_second_booster = self.cases_deaths_by_age_second_booster.loc[
        ...
        helper_preprocess_data_by_second_booster(
            data=deaths_by_second_booster,
            earliest_date="03/27/2022",
            latest_date="09/03/2022",
            output_file_name="deaths_by_second_booster.csv",
        )

        def helper_preprocess_data_by_bivalent_booster(
            data, earliest_date, latest_date, output_file_name
        ):
            """This is a helper function to preprocess the data on first booster vaccination,
            primary series vaccination, and unvaccinated individuals."""
            ...
            for i in range(0, len(data), 2):
                ...
                unvaccinated_multiplier = 1 / (
                    1
                    + (1 / age_adjusted_vax_irr)
                    + (1 / age_adjusted_bivalent_booster_irr)
                )
                data["UV_multiplier"].iloc[i] = unvaccinated_multiplier
                ...
                primary_series_vaccinated_multiplier = 1 / (
                    age_adjusted_vax_irr
                    + 1
                    + (age_adjusted_vax_irr / age_adjusted_bivalent_booster_irr)
                )
                ...
                bivalent_booster_vaccinated_multiplier = 1 / (
                    ...
                    + (age_adjusted_bivalent_booster_irr / age_adjusted_vax_irr)
                    + 1
                )
                data["BiV_multiplier"].iloc[i] = bivalent_booster_vaccinated_multiplier
                data["BiV_multiplier"].iloc[
                    i + 1
        ...
        cases_by_bivalent_booster = self.cases_deaths_by_age_bivalent_booster.loc[
            (self.cases_deaths_by_age_bivalent_booster["outcome"] == "case")
            & (self.cases_deaths_by_age_bivalent_booster["age_group"] == "all_ages")
            & (self.cases_deaths_by_age_bivalent_booster["mmwr_week"] >= 202238)
        ]
        cases_by_bivalent_booster["week_end_date"] = cases_by_bivalent_booster[
            "mmwr_week"
        ].apply(lambda x: epiweeks.Week.fromstring(str(x)).enddate())

        helper_preprocess_data_by_bivalent_booster(
            data=cases_by_bivalent_booster,
            earliest_date="09/17/2022",
            latest_date="03/25/2023",
            output_file_name="cases_by_bivalent_booster.csv",
        )

        # Deaths by bivalent booster.
        deaths_by_bivalent_booster = self.cases_deaths_by_age_bivalent_booster.loc[
            (self.cases_deaths_by_age_bivalent_booster["outcome"] == "death")
            & (self.cases_deaths_by_age_bivalent_booster["age_group"] == "all_ages")
            & (self.cases_deaths_by_age_bivalent_booster["mmwr_week"] >= 202238)
        ]
        deaths_by_bivalent_booster["week_end_date"] = deaths_by_bivalent_booster[
            "mmwr_week"
        ].apply(lambda x: epiweeks.Week.fromstring(str(x)).enddate())

        helper_preprocess_data_by_bivalent_booster(
            data=deaths_by_bivalent_booster,
            earliest_date="09/17/2022",
    ...
    def preprocess_data_by_age_group_vaccination_status(self):
        ...
        def helper_preprocess_data_by_age_group_vaccination_status(
            ...
            for i in range(len(data)):
                age_adjusted_irr = data["Crude IRR"].iloc[i]
                unvaccinated_multiplier = age_adjusted_irr / (age_adjusted_irr + 1)
                data["UV_multiplier"].iloc[i] = unvaccinated_multiplier
                primary_series_vaccinated_multiplier = 1 - unvaccinated_multiplier
                data["PSV_multiplier"].iloc[i] = primary_series_vaccinated_multiplier
        ...
        # Cases by vaccination.
        cases_by_12_17_vaccination = self.cases_deaths_by_age_vaccination.loc[
            (self.cases_deaths_by_age_vaccination["outcome"] == "case")
            & (self.cases_deaths_by_age_vaccination["Age group"] == "12-17")
            & (self.cases_deaths_by_age_vaccination["Vaccine product"] == "all_types")
        ]

        helper_preprocess_data_by_age_group_vaccination_status(
            data=self.cases_deaths_by_age_vaccination,
            outcome="case",
            age_groups=[1],
            vaccination_groups=[1],
            incident_rate_ratios=["Crude IRR"],
            earliest_date="04/04/2021",
            latest_date="09/24/2022",
            output_file_name="cases_by_12_17_vaccination.csv",
        )
...
data__paths = {
    ...
    f"vaccination_effectiveness_and_breakthrough_surveillance/cases_deaths_by_age_second_booster.csv",
    "cases_deaths_by_age_bivalent_booster": f"{data_directory}/cdc/"
    f"vaccination_effectiveness_and_breakthrough_surveillance/cases_deaths_by_age_bivalent_booster.csv",
    "hospitalizations_by_vaccination": f"{data_directory}/cdc/"
    f"vaccination_effectiveness_and_breakthrough_surveillance/hospitalizations_by_vaccination.csv",
}
