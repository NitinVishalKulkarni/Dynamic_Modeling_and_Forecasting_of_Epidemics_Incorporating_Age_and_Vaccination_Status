...
start = time.time()
model.learn(total_timesteps=1 * 10000)
...
for _ in range(100):
    test_observation = test_environment.reset()
    ...
    while not np.any(dones):
        action, _states = model.predict(zero_completed_observation)

        test_observation, rewards, dones, info = test_environment.step(action)
        total_reward += 1
